...
"""
This script processes Templates 1, 2, 3, 4, and 5 accumulatively. It first calculates and saves the Core Metrics (Template 1), then proceeds to calculate and save the Group Code Metrics (Template 2), followed by the Course Rankings (Template 3), calculates and saves the Size Distribution (Template 4), and finally calculates and saves the Detailed Stats (Template 5).
...
if processed_data is not None and column_mapping is not None:
    ...
    # Calculate Detailed Stats (Template 5)
    detailed_stats = processed_data.groupby(["CourseCode", "CourseName", "College"]).agg({
        "Survey1": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey2": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey3": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey4": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey5": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey6": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)],
        "Survey7": ["median", "std", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)]
    }).reset_index()

    # Flatten MultiIndex columns
    detailed_stats.columns = [
        "CourseCode", "CourseName", "College",
        "Survey1_Median", "Survey1_Std", "Survey1_Q1", "Survey1_Q3",
        "Survey2_Median", "Survey2_Std", "Survey2_Q1", "Survey2_Q3",
        "Survey3_Median", "Survey3_Std", "Survey3_Q1", "Survey3_Q3",
        "Survey4_Median", "Survey4_Std", "Survey4_Q1", "Survey4_Q3",
        "Survey5_Median", "Survey5_Std", "Survey5_Q1", "Survey5_Q3",
        "Survey6_Median", "Survey6_Std", "Survey6_Q1", "Survey6_Q3",
        "Survey7_Median", "Survey7_Std", "Survey7_Q1", "Survey7_Q3"
    ]

    # Save Detailed Stats template
    detailed_stats.to_csv(os.path.join(output_dir, "5_standardized-detailed-stats-populated.csv"), index=False)
    print("Detailed Stats template populated and saved.")
...
print("Setup, preprocessing, and Templates 1, 2, 3, 4 & 5 processing completed. Normalized data and templates saved.")
