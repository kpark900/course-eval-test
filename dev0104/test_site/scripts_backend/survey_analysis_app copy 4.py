...
class SurveyDataAnalyzer:
    ...
    def get_top_courses(self, n: int = 5, min_responses: int = 5) -> Optional[pd.DataFrame]:
        """
        Get top n courses by weighted score (average score × number of respondents).
        Only includes courses with at least min_responses respondents.
        """
        if self.data is None or self.column_types is None:
            return None
        ...
        try:
            ...
            course_info = self.data.groupby('course_name').agg({
                'college': 'first',  # Get the college for each course
                'year': 'first',     # Get the year
                'semester': 'first'  # Get the semester
            })
            ...
            # Calculate overall average and response count
            avg_scores = course_stats.xs('mean', axis=1, level=1).mean(axis=1)
            response_counts = course_stats.xs('count', axis=1, level=1).mean(axis=1).round().astype(int)
            
            # Combine into dataframe with course info
            top_courses = pd.DataFrame({
                '년도': course_info['year'],
                '학기': course_info['semester'],
                '개설단과대학': course_info['college'],
                '평균 점수': avg_scores,
                '응답자 수': response_counts,
            })
            
            # Filter for minimum number of responses
            top_courses = top_courses[top_courses['응답자 수'] >= min_responses]
            
            # Calculate weighted score
            top_courses['가중 점수'] = (top_courses['평균 점수'] * top_courses['응답자 수']).round(1)
            
            # Sort by weighted score and get top n
            result = (top_courses
                     .sort_values('가중 점수', ascending=False)
                     .head(n)
                     .round(2))
            
            # Reset index to show course_name as a column
            result = result.reset_index().rename(columns={'course_name': '교과목명'})
            
            # Reorder columns
            column_order = ['년도', '학기', '교과목명', '개설단과대학', '평균 점수', '응답자 수', '가중 점수']
...
def main():
    ...
    if uploaded_file is not None:
        if analyzer.load_data(uploaded_file):
            ...
            # Add min_responses slider
            min_responses = st.slider(
                "최소 응답자 수",
                min_value=1,
                max_value=50,
                value=5,
                help="분석에 포함할 강좌의 최소 응답자 수를 선택하세요"
            )
            
            top_courses = analyzer.get_top_courses(min_responses=min_responses)
            
            if top_courses is not None and not top_courses.empty:
                st.subheader(f"상위 5개 강좌 (최소 응답자 수: {min_responses}명)")
                st.dataframe(top_courses)
                st.caption("가중 점수 = 평균 점수 × 응답자 수")
            else:
                st.warning(f"선택한 최소 응답자 수({min_responses}명)를 만족하는 강좌가 없습니다.")
