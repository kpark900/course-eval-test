...
class MetricsCalculator:
    def __init__(self, data_file: str):
        ...
        self.required_columns = self.survey_cols + ["College", "Campus", "GroupCode", "CourseCode", "Department"]
    ...
    def calculate_additional_metrics(self) -> Dict:
        metrics = {
            'average_scores': {
                col: {
                    'by_department': self.df.groupby('Department')[col].mean().round(2).to_dict(),
            ...
            'course_name_keywords': Counter(
                word.lower()
                for name in self.df['CourseCode']
...
def main():
    ...
    for col, metrics in additional_metrics['average_scores'].items():
        pd.DataFrame(metrics).to_csv(f'{output_dir}/average_scores_{col}.csv')

    for col, metrics in additional_metrics['top_10_colleges'].items():
        pd.DataFrame.from_dict(metrics, orient='index', columns=['Score']).to_csv(f'{output_dir}/top_10_colleges_{col}.csv')

    for col, metrics in additional_metrics['top_10_courses'].items():
        pd.DataFrame.from_dict(metrics, orient='index', columns=['Score']).to_csv(f'{output_dir}/top_10_courses_{col}.csv')

    pd.DataFrame(additional_metrics['course_name_keywords'], columns=['Keyword', 'Frequency']).to_csv(f'{output_dir}/course_name_keywords.csv', index=False)
